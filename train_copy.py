...
from datasets.PowerFlowData_2 import PowerFlowData, random_bus_type
from networks.MPN import MPN, MPN_simplenet, SkipMPN, MaskEmbdMPN, MultiConvNet, MultiMPN, MaskEmbdMultiMPN , MaskEmbdMultiMPN_NNConv , MaskEmbdMultiMPN_GATv2,MaskEmbdMultiMPN_NNConv_v2,MaskEmbdMultiMPN_PhysicsAttn,MaskEmbdMultiMPN_TAG_NNConv,ImprovedPowerFlowGNN 
...
def main():
    ...
    if args.train_loss_fn == 'power_imbalance': # 功率不平衡损失
        ...
        loss_fn = Weighted_Masked_L2_loss(weights=[1.0, 20.0, 1.0, 1.0]).to(device)
        eval_loss_fn = Weighted_Masked_L2_loss(weights=[1.0, 20.0, 1.0, 1.0]).to(device)
    else:
